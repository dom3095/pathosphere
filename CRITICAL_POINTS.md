...
## CP-021: ordine greedy in story-linking blocca merge validi quando un'entità è quasi-hub — **APERTO, non bloccante**
...
**Perché non bloccante**: non è una regressione — `story.py` produce risultati corretti nella stragrande maggioranza dei casi (27 storie nuove formate correttamente in questo run, nessun mega-blob riapparso). È una limitazione di qualità nota (sub-ottimalità dell'ordine greedy con entità quasi-hub), non un bug di correttezza che rompe qualcosa.

**Non fatto in questa sessione**: nessun fix — l'utente ha scelto di investigare la scala prima di decidere se intervenire, e i risultati (natura sovrastimata del numero, rischio di introdurre nuovi problemi con un riordino affrettato) supportano rimandare a una sessione dedicata con più tempo per verifica empirica, invece di un fix rapido su un algoritmo già delicato (già a v3 dopo due iterazioni fallite in precedenza). Possibili direzioni future: ordinare i merge per similarità decrescente invece che per gap temporale crescente; oppure una passata iterativa (ri-tentare le coppie respinte dopo che il grafo si stabilizza) invece di greedy single-pass.
